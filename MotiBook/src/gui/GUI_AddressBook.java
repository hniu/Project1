...
public class GUI_AddressBook {
	...
	/**
	 * printer to print the message
	 */
	public void printMailAddress(){
		// the message box to show the mailling format
		MessageBox mb = new MessageBox(shellAB, SWT.OK);
		if(selected == null)
		{
			mb.setMessage("No Record Selected!");
		...
		else
		{
			...
			mb.setText("Print Message");
			int response=mb.open();
			if(response==SWT.OK)
			{
			       PrintDialog dialog = new PrintDialog(shellAB); 
			       ...
			       { 
			           ...
			           if(printer.startJob("Text")) 
			           {
			               ...
			               { 
			                    gc.drawString(selected.printMailFormat(), 200, 200); 
			                    printer.endPage(); 
			               } 
			               gc.dispose(); 
			               printer.endJob(); 
